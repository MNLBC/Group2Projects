...
--------------------------------------------------------
  ...
  CREATE TABLE "SYSTEM"."CHAT_SESSION" 
   (	"SESSION_ID" NUMBER(3,0), 
	"INVOLVED_CLIENTS" VARCHAR2(500 BYTE), 
...
--------------------------------------------------------
  ...
  ALTER TABLE "SYSTEM"."CHAT_SESSION" MODIFY ("ACTIVE" NOT NULL ENABLE);
...
BEGIN
  ...
  BEGIN
    IF INSERTING AND :NEW.SESSION_ID IS NULL THEN
      SELECT CHAT_SESSION_SEQ1.NEXTVAL INTO :NEW.SESSION_ID FROM SYS.DUAL;
    END IF;
