...
Ext.define('MyApp.controller.controllerMain', {
    ...
    onBtnLoginClick: function(button, e, eOpts) {
        if (button) {
            ...
            var adminView = Ext.getCmp('containerPanel4');
            var admin1 = Ext.getCmp('containerPanel3');
            var admin2 = Ext.getCmp('containerPanel2');
            var admin3 = Ext.getCmp('containerPanel1');
            var btn1 = Ext.getCmp('btnLogout');
            var btn2 = Ext.getCmp('btnSearch');
    ...
    onBtnCustCreateClick: function(button, e, eOpts) {
        var custGrid = this.getCustGrid();
        var custStore = custGrid.getStore();

        var custModel = Ext.create('MyApp.model.Users');
        custModel.set("FirstName", "FirstName");
        custModel.set("LastName", "LastName");
        custModel.set("Email", "Email");
        custModel.set("Password", "Password");
        custModel.set("Birthdate", "Bday");
        custModel.set("Address", "Address");
        custModel.set("City", "City");
        custModel.set("Country", "Country");

        custStore.add(custModel);

        custGrid.editingPlugin.startEdit(custModel, 1);
    },

    onBtnCustDeleteClick: function(button, e, eOpts) {
        var custGrid = this.getCustGrid();
                var custStore = custGrid.getStore();

                //delete selected rows if selModel is checkboxmodel
                var selectedRows = custGrid.getSelectionModel().getSelection();

                if (selectedRows.length)
                    custStore.remove(selectedRows);
                else
                    Ext.Msg.alert('Status', 'Please select at least one record to delete!');
    },

    onBtnCustCloseClick: function(button, e, eOpts) {

    },

    onBtnProdCreateClick: function(button, e, eOpts) {
           var prodGrid = this.getProdGrid();
                        var prodStore = prodGrid.getStore();

                       var prodModel = Ext.create('MyApp.model.Products');
                       prodModel.set("ProductName", "Name");
                       prodModel.set("ProductDescription", "Description");
                       prodModel.set("ProductPrice", "Price");


                       prodStore.add(prodModel);

                       prodGrid.editingPlugin.startEdit(prodModel, 1);
    },

    onBtnProdDeleteClick: function(button, e, eOpts) {
        var prodGrid = this.getProdGrid();
                var prodStore = prodGrid.getStore();

                //delete selected rows if selModel is checkboxmodel
                var selectedRows = prodGrid.getSelectionModel().getSelection();

                if (selectedRows.length)
                    prodStore.remove(selectedRows);
                else
                    Ext.Msg.alert('Status', 'Please select at least one record to delete!');
    },
    ...
    init: function(application) {
        this.control({
            ...
            "#btnCustCreate": {
                click: this.onBtnCustCreateClick
            },
            "#btnCustDelete": {
                click: this.onBtnCustDeleteClick
            },
            "#btnCustClose": {
                click: this.onBtnCustCloseClick
            },
            "#btnProdCreate": {
                click: this.onBtnProdCreateClick
            },
            "#btnProdDelete": {
                click: this.onBtnProdDeleteClick
            }
        });
    }
