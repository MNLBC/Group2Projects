            ...
            <xsl:for-each
              ...
              <xsl:choose>
                <xsl:when test="position() = 1">
                  <xsl:for-each select="satellite-information/satellite">
                    ...
                    <xsl:if test="management-address">
                      <managementAddress>
                        <xsl:value-of
                          select="normalize-space(management-address)" />
                      </managementAddress>
                    </xsl:if>
                  </xsl:for-each>
                </xsl:when>
                <xsl:when test="position() = 2">
                  <xsl:for-each
                    select="upgrade-group-information/upgrade-group">
                    ...
                    <xsl:for-each select="connectivity">
                      <connectivity>
                        <xsl:if test="cascade-port">
                          <cascadePort>
                            <xsl:value-of
                              select="normalize-space(cascade-port)" />
                          </cascadePort>
                        </xsl:if>
                        <xsl:if test="uplink-interface-name">
                          <uplinkIntefaceName>
                            <xsl:value-of
                              select="normalize-space(uplink-interface-name)" />
                          </uplinkIntefaceName>
                        </xsl:if>
                        <xsl:if test="adjacency-state">
                          <adjacency-state>
                            <xsl:value-of
                              select="normalize-space(adjacency-state)" />
                          </adjacency-state>
                        </xsl:if>
                      </connectivity>
                    </xsl:for-each>
